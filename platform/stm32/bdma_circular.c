...
/*----------------------------------------------------------------------------*/
const struct DmaClass * const BdmaCircular = &(const struct DmaClass){
    .size = sizeof(struct BdmaCircular),
    .init = streamInit,
    .deinit = streamDeinit,
...
{
  const struct BdmaCircularConfig * const config = configBase;
  assert(config != NULL);
    ...
    size_t size)
{
  ...
  assert(!(stream->base.config & CCR_HTIE) || (size % 2 == 0));
...
{
  ((struct BdmaCircular *)object)->state = STATE_IDLE;
}
/*----------------------------------------------------------------------------*/
