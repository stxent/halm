...
#include <halm/platform/bouffalo/dma_list.h>
...
static void startTransfer(struct DmaList *, const struct DmaEntry *);
/*----------------------------------------------------------------------------*/
...
{
  struct DmaList * const channel = object;
  bool event = false;
...
/*----------------------------------------------------------------------------*/
static void startTransfer(struct DmaList *channel,
...
{
  const struct DmaList * const channel = object;
