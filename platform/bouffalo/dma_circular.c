...
/*----------------------------------------------------------------------------*/
static size_t getCurrentEntry(const struct DmaCircular *);
static void interruptHandler(void *, enum Result);
static void startTransfer(struct DmaCircular *, const struct DmaEntry *);
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
static void startTransfer(struct DmaCircular *channel,
    const struct DmaEntry *entry)
{
  BL_DMA_CHANNEL_Type * const reg = channel->base.reg;

  reg->SRCADDR = entry->source;
  reg->DSTADDR = entry->destination;
    ...
    void *argument)
{
  ...
  for (size_t index = 0; index < channel->queued; ++index)
  {
    if (channel->callback && (!channel->silent || index == channel->queued - 1))
      channel->list[index].control |= CONTROL_TCIEN;
    else
      channel->list[index].control &= ~CONTROL_TCIEN;
  }
}
