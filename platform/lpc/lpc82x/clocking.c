...
#include <halm/platform/lpc/lpc82x/clocking_defs.h>
...
static void configClockPin(PinNumber);
...
/*----------------------------------------------------------------------------*/
static void configClockPin(PinNumber key)
{
...
{
  static const PinNumber xtalInPin = PIN(0, 8);
  static const PinNumber xtalOutPin = PIN(0, 9);

  configClockPin(xtalInPin);
  if (!bypass)
    configClockPin(xtalOutPin);
}
/*----------------------------------------------------------------------------*/
static void extOscDisable(const void *)
{
...
static enum Result clockOutputEnable(const void *, const void *configBase)
{
  ...
  struct Pin clkout = pinInit(config->pin);

  pinOutput(clkout, false);
  pinSetMux(clkout, PINMUX_CLKOUT);
