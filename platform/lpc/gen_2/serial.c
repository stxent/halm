...
static void interruptHandler(void *object)
{
  ...
  bool event = (reg->STAT & STAT_RXIDLE) != 0;

  if (status & INTSTAT_RXRDY)
  {
    const uint8_t data = reg->RXDATA;
  ...
  if ((reg->INTENSET & INTENSET_TXRDYEN) && (status & INTSTAT_TXRDY))
  {
    updateTxWatermark(interface, byteQueueSize(&interface->txQueue));

    if (!byteQueueEmpty(&interface->txQueue))
      reg->TXDATA = byteQueuePopFront(&interface->txQueue);

    if (byteQueueEmpty(&interface->txQueue))
    {
      reg->INTENCLR = INTENCLR_TXRDYCLR;
      event = true;
    }
  }
