...
{
  ((LPC_GPIO_Type *)gpio.reg)->MASKED_ACCESS[1 << gpio.pin.offset] = 0x000;
}
/*----------------------------------------------------------------------------*/
static inline void gpioSet(struct Gpio gpio)
{
  ((LPC_GPIO_Type *)gpio.reg)->MASKED_ACCESS[1 << gpio.pin.offset] = 0xFFF;
}
/*----------------------------------------------------------------------------*/
static inline void gpioWrite(struct Gpio gpio, uint8_t value)
{
  ((LPC_GPIO_Type *)gpio.reg)->MASKED_ACCESS[1 << gpio.pin.offset] =
      value ? 0xFFF : 0x000;
}
/*----------------------------------------------------------------------------*/
